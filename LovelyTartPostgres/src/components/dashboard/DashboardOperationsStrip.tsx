...
export const DashboardOperationsStrip: React.FC<DashboardOperationsStripProps> = ({ onNavigate }) => {
  return (
    <section className="crm-chart-panel">
    <div className="crm-section-header">
      ...
      <div className="crm-status-tag bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border-emerald-200/60 dark:border-emerald-800/40 self-start sm:self-center shrink-0">
      ...
      {operationalAreas.map((area) => {
        ...
        return (
          <button
            key={area.id}
            type="button"
            className="flex flex-col text-left p-3.5 rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-800/80 hover:border-blue-400/50 dark:hover:border-blue-500/50 hover:shadow-md transition-all group cursor-pointer"
          ...
          >
            <div className="flex items-center justify-between gap-2 mb-2 w-full">
              ...
              <span className="w-6 h-6 rounded-md bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                <AreaIcon size={13} />
              </span>
            </div>
            ...
            <div className="mt-auto pt-2 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between w-full">
              <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded border ${toneStyles}`}>
                <CheckCircle2 size={10} />
                {area.status}
              </span>
              <ArrowUpRight size={12} className="text-slate-400 dark:text-slate-500 group-hover:text-blue-500 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
            </div>
          </button>
        );
      })}
    ...
    <div className="p-3.5 rounded-xl bg-slate-50/80 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800/70">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">· 16 módulos unificados</span>
        </div>
